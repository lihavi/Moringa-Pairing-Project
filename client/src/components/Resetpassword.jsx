...
function ResetPassword() {
  ...
  const handleResetPassword = async (event) => {
    ...
    try {
      const response = await axios.post('https://moringa-pair-akon.onrender.com/reset_password', { email });
  ...
  const handleSetNewPassword = async (event) => {
    event.preventDefault();
    try {
      const response = await axios.put(`https://moringa-pair-akon.onrender.com/update_password/${resetToken}`, {
    ...
    <div className="container pt-0">
      <div className="row full-height justify-content-center pt-0">
        <div className="col-12 text-center align-self-center py-5 pt-0">
          <div className="section pb-0 pt-0 pt-sm-0 text-center pt-0">
            <div className="card-3d-wrap mx-auto pt-5" style={{backgroundColor: 'white'}}>
      ...
      {!resetToken ? (
        <form onSubmit={handleResetPassword}>
          ...
          <button className=" mt-4 btn btn-outline-primary" type="submit">Reset Password</button>
        </form>
      ) : (
        <form onSubmit={handleSetNewPassword}>
         <div className="form-group">
          ...
          <button className="btn mt-4 btn btn-outline-primary" type="submit">Set New Password</button>
        </form>
      )}
    </div>
    </div>
